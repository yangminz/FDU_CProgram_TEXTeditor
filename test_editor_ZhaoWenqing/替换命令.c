...
void replaceStr(void)			/* 替换（删除）字符串的函数	*/
{
	...
	else if(numStr == 3 && cmdStr[1][0] == '\0' &&  cmdStr[2][0] == 'g')
	{
		...
		for(ThisLine=1, node=Head->next; node; node=node->next, ThisLine++)
		{
			match = 0;
			kchar = 0;
			while( (ichar = substr(&(node->textline[kchar]), cmdStr[0])) != EOF )
			{
				kchar += ichar;
				for(i=0; node->textline[i+kchar+lenStr]!='\0'; i++)
					node->textline[i+kchar] =
					node->textline[i+kchar+lenStr];
				node->textline[i+kchar] = '\0';
				fail = 0;
				match = 1;
			}
			if(match)
	...
	else if(numStr == 3 && cmdStr[1][0] != '\0' &&  cmdStr[2][0] == 'g')
	{
		...
		for(ThisLine=1, node=Head->next; node; node=node->next, ThisLine++)
		{
			...
			while( (ichar = substr(&(node->textline[kchar]), cmdStr[0])) != EOF )
			{
				lenText -= ichar;
				kchar += ichar;
				repstr(	&(node->textline[kchar]), lenText,
						cmdStr[0], lenStr, cmdStr[1], lenTar);
				lenText -= lenStr;
				kchar += lenTar;
				fail = 0;
